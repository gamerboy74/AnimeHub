...
export async function scrapeAndSaveEpisode(anime, ep) {
  ...
  try {
    console.log(`    [1/4] NineAnime checking EP ${ep}...`);
    nineAnimeRes = await enqueue(() =>
      NineAnimeScraperService.scrapeAnimeEpisode(
        animeTitle,
        ep,
        { timeout: 45000, retries: 2, dbAnimeId: anime.id }
      ),
      "high"
    );
  ...
  try {
    console.log(`    [2/4] AnimeSuge checking EP ${ep}...`);
    ...
    if (isNotFoundCached) {
      ...
      if (animeSugeRes?.success) {
        ...
        if (errorMsg.includes("Could not find a secure search result") || errorMsg.includes("No results found")) {
          try {
  ...
  let reAnimeRes = null;
  try {
    console.log(`    [3/4] Re:ANIME checking EP ${ep}...`);
  ...
  try {
    console.log(`    [4/4] SanjiAnime checking EP ${ep}...`);
    ...
    if (isNotFoundCached) {
      ...
      if (sanjiAnimeRes?.success) {
        ...
        if (list.length > 0) {
          newServers.push(...list);
          console.log(`    ✅ SanjiAnime found ${list.length} stream URL(s)`);
        } else {
  ...
  if (uniqueNewServers.length > 0) {
    ...
    if (existingEpisode) {
      ...
      savedEpisode = data;
      console.log(`  💾 Updated existing EP ${ep} in database`);
    } else {
      const insertPayload = {
        anime_id: anime.id,
        episode_number: ep,
